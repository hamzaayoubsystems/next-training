...
export default async function handler(
  ...
  const indianCuisineId =
    cuisines.find((cuisine: { name: string }) => cuisine.name === "indian")
      ?.id || 1;
  const mexicanCuisineId =
    cuisines.find((cuisine: { name: string }) => cuisine.name === "mexican")
      ?.id || 1;
  const italianCuisineId =
    cuisines.find((cuisine: { name: string }) => cuisine.name === "italian")
      ?.id || 1;

  const ottawaLocationId =
    locations.find((location: { name: string }) => location.name === "ottawa")
      ?.id || 1;
  const torontoLocationId =
    locations.find((location: { name: string }) => location.name === "toronto")
      ?.id || 1;
  const niagaraLocationId =
    locations.find((location: { name: string }) => location.name === "niagara")
      ?.id || 1;
  ...
  const vivaanId =
    restaurants.find(
      (restaurant: { name: string }) =>
        restaurant.name === "Vivaan - fine Indian"
    )?.id || 1;
  const RamaKrishnaId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "RamaKrishna Indian"
    )?.id || 1;
  const coconutLagoonId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Coconut Lagoon"
    )?.id || 1;
  const lastTrainToDelhiId =
    restaurants.find(
      (restaurant: { name: string }) =>
        restaurant.name === "Last Train to Delhi"
    )?.id || 1;
  const adrakYorkvilleId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Adrak Yorkville"
    )?.id || 1;
  const curryishTavernId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Curryish Tavern"
    )?.id || 1;
  const utsavId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Utsav"
    )?.id || 1;
  const pukkaId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Pukka"
    )?.id || 1;
  const kamasutraIndianId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Kamasutra Indian"
    )?.id || 1;
  const eldoradoTacoId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Eldorado Taco"
    )?.id || 1;
  const laBartolaId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "La Bartola"
    )?.id || 1;
  const elCatrinId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "El Catrin"
    )?.id || 1;
  const mariachisId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "3 Mariachis"
    )?.id || 1;
  const canoRestaurantId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Cano Restaurant"
    )?.id || 1;
  const bluRistoranteId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Blu Ristorante"
    )?.id || 1;
  const stelvioId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Stelvio"
    )?.id || 1;
  const sofiaId =
    restaurants.find(
      (restaurant: { name: string }) => restaurant.name === "Sofia"
    )?.id || 1;
